// Поиск потока на одном уровне пирамиды: полный перебор в радиусе gRadius вокруг предсказания
// (поток грубого уровня, если FLAG_HAS_PRIOR) + отдельный кандидат «нет движения».
// Выход: xy — вектор A->B в UV, z — стоимость совпадения (для отладки/доверия).
...
void main(uint3 id : SV_DispatchThreadID)
{
    ...
    // Нулевой вектор: статичный фон и HUD не должны «плыть».
