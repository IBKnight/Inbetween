// Общие объявления для ВСЕХ шейдеров. Подключается через #include "common.hlsli".
// cbuffer Params ОБЯЗАН совпадать с gfx.Params (internal/gfx/params.go), 96 байт.
#ifndef COMMON_HLSLI
#define COMMON_HLSLI

// Размер группы задаётся из Go (gfx.Shader.GroupX/GroupY) через макросы.
#ifndef GROUP_X
#define GROUP_X 8
#endif
#ifndef GROUP_Y
...
cbuffer Params : register(b0)
{
    ...
    uint2  gDstSize;     // c1.xy  размер записываемой текстуры
    float2 gInvDstSize;  // c1.zw
    float  gT;           // c2.x   время интерполяции 0..1
    float  gTime;        // c2.y   секунды (синтетика)
    uint   gLevel;       // c2.z
    uint   gRadius;      // c2.w
    uint   gFlags;       // c3.x
...
// Центр пикселя p в UV записываемой текстуры.
